...
<html lang="en">
    <head>
        ...
        <script>
            $(document).ready(function(){
                ...
                //     // return false;
                // });
                $("#getStationbutton").click(function() {
                    $('getStationbutton').submit(function() {
                        $.get($(this).attr('action'), $(this).serialize(), function(res) {
                            // var html_string2 = "";
                            // if(res.results.length !== 0) {
                            //     var_dump(res.results[0]);
        ...
        <link rel="stylesheet" href="/assets/owl/owl-redux2.carousel.css">
    ...
    <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-md-9 col-lg-10 order-1 order-md-2 content" style="margin: 0px 0px 0px 213px; background-color: #f8f9fa;">
                    ...
                    <div class = "row">
                        ...
                        ?>
                        <?php
                            // var_dump($this->session->userdata('Prestine'));
                            // var_dump($this->session->userdata('Station'));
                            // var_dump($this->session->userdata('StationTunes'));
                            if($this->session->userdata('Station') != null) {
                                echo "  <h4 style='margin: 0px 0px 0px 430px;'>
                                            <b>" . $this->session->userdata('Station')['title'] . "</b>
                                        ...
                                        <br>";
                                echo "<img style='width: 160px; height: 187px; border-radius:50%; margin: 0px 0px 25px 430px;' src=" . $this->session->userdata('Station')['picture'] . ">";
                        ...
                        ?>
                            <form id="tunes" action="/Song/getStationTunes" method="GET">
                                <button style="margin: 0px 0px 0px 471px;" class="superbtn btn1" id="getStationbutton"><?= $this->session->userdata('Station')['title'] ?></button>
                            </form>
                            <?php if($this->session->userdata('Prestine') == FALSE) { ?>
                                <script>
                                    window.onload=function(){
                                        document.getElementById("getStationbutton").click();
                                    };
                                </script>
                            <?php } ?>
                        </div>
        ...
        try{
                if($this->session->userdata('StationTunes') != null) { ?>
                    <div id="top-songs" class="owl-carousel owl-theme" >
                    <?php 
                    for($v= 0; $v < count($this->session->userdata('StationTunes')['data']); $v++){
                        // var_dump($this->session->userdata('StationTunes')['data'][$v]['album']);
                        ?>
                        <!-- <div class="item text-center"> -->
                        
                        <br><div class = 'item text-center'><ul style='list-style-type: none; justify-content: center; align-items: center;'><li><h4><b><?= $this->session->userdata('StationTunes')['data'][$v]['title'] ?></b><i style="margin: 0px 0px 0px 7px;" class='fa fa-music'></i></h4></li><br>
                        <li><img src='<?= $this->session->userdata('StationTunes')['data'][$v]['artist']['picture'] ?>'  ></li>
                        <li><h5><form action=/Song/getArtist   method=POST><input type=hidden name=Artist value='<?= $this->session->userdata('StationTunes')['data'][$v]['artist']['id'] ?>' ><input id=getArtistbutton type=submit value='<?= $this->session->userdata('StationTunes')['data'][$v]['artist']['name'] ?>' ></form></h5></li>
                        
                            <li><form id=getStationbutton  action=/Station/getAlbum  method=POST>
                                        <input type=hidden name=songAlbum value='<?= $this->session->userdata('StationTunes')['data'][$v]['album']['tracklist'] ?>'>
                                        <input type=hidden name=thisAlbumTitle value='<?= $this->session->userdata('StationTunes')['data'][$v]['album']['title'] ?>'>
                                        <input type=hidden name=thisAlbumCover value='<?= $this->session->userdata('StationTunes')['data'][$v]['album']['cover'] ?>'>
                                        <input type=hidden name=album value='<?= $v ?>'>
                                        <input type="hidden" name="AlbumView" value="Yes">
                                        <?php if($v == $this->session->userdata('3count3')){ ?>
                                            <input id=getArtistbutton7  type = submit value = '<?= $this->session->userdata('StationTunes')['data'][$v]['album']['title'] ?>'>
                                        <?php }else{ ?>
                                            <input id=getArtistbutton  type = submit value = '<?= $this->session->userdata('StationTunes')['data'][$v]['album']['title'] ?>'>
                                        <?php } ?>
                                    </form></li>
                                    <li><img class=image1234 src='<?= $this->session->userdata('StationTunes')['data'][$v]['album']['cover'] ?>'><li>
                                        <li><audio src='<?=$this->session->userdata('StationTunes')['data'][$v]['preview']?>'    controls></audio></li></ul>
    ...
    ?>
            ...
            <div id="ThisAlbums" class="owl-carousel owl-theme" >
            <?php
                // echo "Test!123!";
                // var_dump($this->session->userdata('AlbumSongs'));
                try{
            ...
            ?>
                            ...
                            <li><audio style="width: 160px;" class=audio123 src='<?=$this->session->userdata('AlbumSongs')['data'][$r]['preview']?>'    controls></audio></li></ul></div>
            <?php
                        }
